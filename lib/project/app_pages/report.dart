...
class Report extends StatefulWidget {
  ...
  final IconData icon;
  const Report({required this.type, required this.category, required this.selectedDate, required this.icon});

  @override
  _ReportState createState() => _ReportState();
}

//essa classe é usada para criar uma tela de relatório que exibe informações financeiras com base no tipo (renda ou despesa),
//categoria e data selecionada pelo usuário.
...
class ReportBody extends StatefulWidget {
  ...
  final Color color;
  final IconData icon;
  ReportBody(this.type, this.category, this.selectedDate, this.color, this.icon);
  @override
    ...
    return ChangeNotifierProvider(
      ...
      builder: (context, child) {
        return FutureBuilder<List<InputModel>>(
          ...
          builder: (BuildContext context, AsyncSnapshot<List<InputModel>> snapshot) {
            connectionUI(snapshot);
            if (snapshot.data == null || snapshot.connectionState == ConnectionState.waiting) {
              return SizedBox();
            } else {
              double yearAmount = 0;
              DateTime date(int duration) => startOfThisYear.add(Duration(days: duration));
              bool isLeapYear(int year) {
                return (year % 4 == 0) && (year % 100 != 0) || (year % 400 == 0);
              }

              // widget.transactions.sort((a, b) => a.date!.compareTo(b.date!));
              List<InputModel> sortByCategory(List<InputModel> data, String type) {
                return data
                    .map((data) {
                      if (data.type == type && data.category == widget.category) {
              ...
              List<InputModel> transactions = widget.type == 'Income' ? sortByCategory(snapshot.data!, 'Income') : sortByCategory(snapshot.data!, 'Expense');

              List<InputModel> transactionsYearly =
                  (transactions
                          .map((data) {
                            DateTime dateSelectedDT = DateFormat('dd/MM/yyyy').parse(data.date!);

                            if (dateSelectedDT.isAfter(startOfThisYear.subtract(Duration(days: 1))) && dateSelectedDT.isBefore(DateTime(todayDT.year, 12, 31))) {
                              return inputModel(data);
                            }
                          })
                          .where((element) => element != null)
              ...
              MonthAmount monthBasedTransaction(String month, DateTime date, int days) {
                double monthAmount = 0;
                for (InputModel transaction in transactionsYearly) {
                  DateTime dateSelectedDT = DateFormat('dd/MM/yyyy').parse(transaction.date!);

                  if (dateSelectedDT.isAfter(date) && dateSelectedDT.isBefore(startOfThisYear.add(Duration(days: days)))) {
                ...
                children: [
                        ...
                        onMarkerRender: (MarkerRenderArgs args) {
                          if (this.widget.type == 'Income') {
                            if (args.pointIndex == 0) {
                              args.color = const Color.fromRGBO(9, 110, 16, 1);
                            } else if (args.pointIndex == 1) {
                              args.color = const Color.fromRGBO(19, 134, 13, 1);
                            } else if (args.pointIndex == 2) {
                              args.color = const Color.fromRGBO(55, 171, 49, 1);
                            } else if (args.pointIndex == 3) {
                              args.color = const Color.fromRGBO(77, 213, 70, 1);
                            } else if (args.pointIndex == 4) {
                              args.color = const Color.fromRGBO(134, 213, 70, 1);
                            } else if (args.pointIndex == 5) {
                              args.color = const Color.fromRGBO(156, 222, 103, 1);
                            } else if (args.pointIndex == 6) {
                              args.color = const Color.fromRGBO(153, 249, 172, 1);
                            } else if (args.pointIndex == 7) {
                              args.color = const Color.fromRGBO(189, 235, 120, 1);
                            } else if (args.pointIndex == 8) {
                              args.color = const Color.fromRGBO(177, 249, 191, 1);
                            } else if (args.pointIndex == 9) {
                              args.color = const Color.fromRGBO(217, 241, 179, 1);
                            } else if (args.pointIndex == 10) {
                              args.color = const Color.fromRGBO(235, 246, 199, 1);
                            } else if (args.pointIndex == 11) {
                              args.color = Colors.white;
                            }
                          } else {
                            if (args.pointIndex == 0) {
                              args.color = const Color.fromRGBO(159, 16, 32, 1);
                            } else if (args.pointIndex == 1) {
                              args.color = const Color.fromRGBO(197, 71, 84, 1);
                            } else if (args.pointIndex == 2) {
                              ...
                              args.color = const Color.fromRGBO(230, 168, 138, 1);
                            } else if (args.pointIndex == 7) {
                              args.color = const Color.fromRGBO(221, 176, 108, 1);
                            } else if (args.pointIndex == 8) {
                              args.color = const Color.fromRGBO(222, 187, 97, 1);
                            } else if (args.pointIndex == 9) {
                              args.color = const Color.fromRGBO(250, 204, 160, 1);
                            } else if (args.pointIndex == 10) {
                              args.color = const Color.fromRGBO(248, 219, 191, 1);
                            } else if (args.pointIndex == 11) {
                              args.color = Colors.white;
                            }
                          }
                  ...
                  ChangeNotifierProvider<ChangeSelectedDate>(
                    ...
                    child: Selector<ChangeSelectedDate, String?>(
                      selector: (_, changeSelectedDate) => changeSelectedDate.selectedReportDate,
                      builder: (context, selectedAnalysisDate, child) {
                        ...
                        List<InputModel> selectedTransactions = filterData(context, transactions, selectedAnalysisDate);
                        double totalAmount = 0;
                        if (selectedTransactions.length > 0) {
                          for (InputModel? transaction in selectedTransactions) {
                        ...
                        return Expanded(
                          child: Column(
                            children: [
                              ...
                              Padding(
                                padding: EdgeInsets.only(left: totalAmount.toString().length < 16 ? 10.w : 6.w, right: totalAmount.toString().length < 15 ? 20.h : 10.h, top: 25.h),
                                child: Row(
                                  children: [
                                    DropDownBox(
                              ...
                              Divider(thickness: 0.5.h, height: 25.h, color: grey),
                              Expanded(
...
// por exibir o formulário de edição.
                                      ...
                                      child: SwipeActionCell(
                                        ...
                                        key: ObjectKey(selectedTransactions[int]),
                                        ...
                                        trailingActions: <SwipeAction>[
                                          ...
                                          SwipeAction(
                                            ...
                                            onTap: (CompletionHandler handler) {
                                              var model = selectedTransactions[int];
                                              model.id = null;
                                              DB.insert(model);
                                              Provider.of<InputModelList>(context, listen: false).changeInputModelList();
                                        ...
                                        child: Padding(
                                          ...
                                          child: Row(
                                            children: [
                                              Text(DateFormat(sharedPrefs.dateFormat).format(DateFormat('dd/MM/yyyy').parse(selectedTransactions[int].date!)), style: GoogleFonts.aBeeZee(fontSize: 17.sp)),
                                              Spacer(),
                                              Text('${format(selectedTransactions[int].amount!)} $currency', style: GoogleFonts.aBeeZee(fontSize: 18.5.sp)),
                                              SizedBox(width: 15.w),
...
List<CartesianSeries<MonthAmount, String>> _getGradientAreaSeries(String type, List<MonthAmount> monthAmountList) {
  return <CartesianSeries<MonthAmount, String>>[
    SplineAreaSeries<MonthAmount, String>(
          borderGradient: type == 'Income' ? const LinearGradient(colors: <Color>[Color.fromRGBO(56, 135, 5, 1), Color.fromRGBO(159, 196, 135, 1)], stops: <double>[0.2, 0.9]) : const LinearGradient(colors: <Color>[Color.fromRGBO(212, 126, 166, 1), Color.fromRGBO(222, 187, 104, 1)], stops: <double>[0.2, 0.9]),
          gradient: type == 'Income' ? const LinearGradient(colors: <Color>[Color.fromRGBO(101, 181, 60, 1), Color.fromRGBO(139, 194, 72, 1), Color.fromRGBO(203, 241, 119, 0.9)], stops: <double>[0.2, 0.5, 0.9]) : const LinearGradient(colors: <Color>[Color.fromRGBO(224, 139, 207, 0.9), Color.fromRGBO(255, 232, 149, 0.9)], stops: <double>[0.2, 0.9]),
          borderWidth: 2.5,
          markerSettings: MarkerSettings(isVisible: true, height: 8, width: 8, borderColor: type == 'Income' ? Color.fromRGBO(161, 171, 35, 1) : Colors.white, borderWidth: 2),
        ...
        as CartesianSeries<MonthAmount, String>, // 👈 Cast explícito aqui
  ];
}
