...
class FormatDate extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    List<String> dateFormats = ['dd/MM/yyyy', 'MM/dd/yyyy', 'yyyy/MM/dd', 'yMMMd', 'MMMEd', 'MEd', 'MMMMd', 'MMMd'];
    return Scaffold(
      appBar: AppBar(
        backgroundColor: blue3,
        title: Text(getTranslated(context, 'Select a date format') ?? 'Select a date format', style: TextStyle(fontSize: 21.sp)),
        actions: [
          Padding(
      ...
      body: ChangeNotifierProvider<OnDateFormatSelected>(
        ...
        builder: (context, widget) => ListView.builder(
          ...
          itemBuilder: (context, int) => GestureDetector(
            ...
            onTap: () {
              context.read<OnDateFormatSelected>().onDateFormatSelected(dateFormats[int]);
            },
            child: Column(
              ...
              children: [
                Padding(
                  ...
                  child: Row(
                    children: [
                      Text('${DateFormat(dateFormats[int]).format(now)}', style: TextStyle(fontSize: 19.sp)),
                      Spacer(),
                      context.watch<OnDateFormatSelected>().dateFormat == dateFormats[int] ? Icon(Icons.check_circle, size: 25.sp, color: blue3) : SizedBox(),
                    ],
                  ),
                ),
