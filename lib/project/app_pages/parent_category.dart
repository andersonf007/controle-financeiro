...
class ParentCategoryList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    List<CategoryItem> parentCategories = sharedPrefs.getAllExpenseItemsLists().map((item) => CategoryItem(item[0].iconCodePoint, item[0].iconFontPackage, item[0].iconFontFamily, item[0].text, item[0].description)).toList();
    return Scaffold(
          ...
          return GestureDetector(
            ...
            child: Column(
              children: [
                  ...
                  child: Row(
                    children: [
                      CircleAvatar(
                        ...
                        radius: 20.r,
                        child: Icon(iconData(parentCategories[int]), size: 25.sp, color: red),
                      ),
                      SizedBox(width: 28.w),
                      Expanded(
                        child: Text(
                          getTranslated(context, parentCategories[int].text) ?? parentCategories[int].text,
                ...
                Divider(thickness: 0.25.h, indent: 67.w, color: grey),
              ],
            ),
          );
        },
