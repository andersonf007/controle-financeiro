...
import 'package:controle_financeiro/project/app_pages/select_date_format.dart';
import 'package:controle_financeiro/project/app_pages/select_language.dart';
import 'package:controle_financeiro/project/auth_pages/user_account.dart';
...
import 'package:controle_financeiro/project/rate_app_service.dart';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: PreferredSize(
        // esse widget é a barra de aplicativo onde tem o título e o ícone de rosto
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                CircleAvatar(
                  child: CircleAvatar(
                    child: Icon(FontAwesomeIcons.faceSmileBeam.data, color: Colors.black, size: 71.sp),
                    radius: 35.r,
                ...
                SizedBox(width: 20.w),
                Text('${getTranslated(context, 'Hi you')!}!', style: TextStyle(fontSize: 30.sp)),
                // Spacer(),
                // Icon(
...
class _SettingsState extends State<Settings> {
  @override
  Widget build(BuildContext context) {
    List<Widget> pageRoute = [UserAccount(), SelectLanguage(), Currency()];
    List<Widget> settingsIcons = [
      Icon(Icons.account_circle, size: 35, color: Colors.lightBlue),
      // Icon(
      //   Icons.settings,
      //   size: 32,
      ...
      // ),
      Icon(Icons.language, size: 32.sp, color: Colors.lightBlue),
      Icon(Icons.monetization_on, size: 32.sp, color: Colors.orangeAccent),
      ...
      Icon(Icons.share, size: 28.sp, color: Colors.lightBlue),
      Icon(Icons.star, size: 32.sp, color: Colors.amber),
    ];
        ...
        return GestureDetector(
          onTap: () async {
            if ((int == 0) || (int == 1) || (int == 2)) {
              Navigator.push(context, MaterialPageRoute(builder: (context) => pageRoute[int]));
            } else if (int == 3) {
              Navigator.push(context, MaterialPageRoute(builder: (context) => FormatDate())).then((value) => setState(() {}));
            } else if (int == 4) {
              // Navigator.push(
              //     context,
              //     MaterialPageRoute(
              ...
              Platform.isIOS ? await iosDialog(context, 'This action cannot be undone. Are you sure you want to reset all categories?', 'Reset', onReset) : await androidDialog(context, 'This action cannot be undone. Are you sure you want to reset all categories?', 'reset', onReset);
            } else if (int == 5) {
              ...
              Platform.isIOS ? await iosDialog(context, 'Deleted data can not be recovered. Are you sure you want to delete all data?', 'Delete', onDeletion) : await androidDialog(context, 'Deleted data can not be recovered. Are you sure you want to delete all data?', 'Delete', onDeletion);
            }
            // else if (int == 4) {
            //   onPasscodeSwitched();
            // }
              ...
              customToast(context, 'Opening app store...');
            }
          },
          child: Column(
            children: [
                  ...
                  child: Center(
                    child: ListTile(
                      title: Padding(
                        ...
                        child: Text('${settingsList[int]}', style: TextStyle(fontSize: 18.5.sp)),
                      ),
                      leading: CircleAvatar(radius: 24.r, backgroundColor: Color.fromRGBO(229, 231, 234, 1), child: settingsIcons[int]),
                      trailing:
                          // int == 4
                          // ? Switch(
                          ...
                          //   ) :
                          Icon(Icons.arrow_forward_ios, size: 20.sp, color: Colors.blueGrey),
                    ),
                  ),
                ),
              ),
              Divider(indent: 78.w, height: 0.1.h, thickness: 0.4.h, color: grey),
            ],
          ),
        );
      },
