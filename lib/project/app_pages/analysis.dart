...
import 'package:controle_financeiro/project/classes/chart_pie.dart';
...
final List<InputModel> chartDataNull = [InputModel(id: null, type: null, amount: 1, category: '', description: null, date: null, time: null, color: const Color.fromRGBO(0, 220, 252, 1))];
      ...
      child: DefaultTabController(
        ...
        child: Scaffold(
          ...
          body: Selector<ChangeSelectedDate, String?>(
            selector: (_, changeSelectedDate) => changeSelectedDate.selectedAnalysisDate,
            builder: (context, selectedAnalysisDate, child) {
              selectedAnalysisDate ??= sharedPrefs.selectedDate;
              ListView listViewChild(String type) => ListView(children: [ShowDate(true, selectedAnalysisDate!), ShowDetails(type, selectedAnalysisDate)]);
              return TabBarView(children: [listViewChild('Expense'), listViewChild('Income')]);
            },
          ),
        ),
      ),
...
class ShowDate extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          Icon(Icons.calendar_today, size: 27.sp, color: Color.fromRGBO(82, 179, 252, 1)),
          SizedBox(width: 10.w),
...
class DateDisplay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    TextStyle style = GoogleFonts.aBeeZee(fontSize: 20.sp, fontWeight: FontWeight.bold);

    Map<String, Widget> dateMap = {
      'Today': Text('$today', style: style),
      'This week': Text('$since ${DateFormat(sharedPrefs.dateFormat).format(startOfThisWeek)}', style: style),
      'This month': Text('$since ${DateFormat(sharedPrefs.dateFormat).format(startOfThisMonth)}', style: style),
      'This quarter': Text('$since ${DateFormat(sharedPrefs.dateFormat).format(startOfThisQuarter)}', style: style),
      'This year': Text('$since ${DateFormat(sharedPrefs.dateFormat).format(startOfThisYear)}', style: style),
  ...
  Widget build(BuildContext context) {
    Widget rowFrame(String typeName, double value) {
      return Row(
        ...
        children: [
          Text(getTranslated(context, typeName)!, style: TextStyle(fontSize: 22.sp)),
          Expanded(
            child: Text(
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: grey, width: 0.4.w),
      ),
      child: Padding(
...
class _ShowDetailsState extends State<ShowDetails> {
  Widget showInExDetails(BuildContext context, List<InputModel> transactionsSorted) {
    List<CategoryItem> itemList = widget.type == 'Income' ? createItemList(transactions: transactionsSorted, forAnalysisPage: true, isIncomeType: true, forSelectIconPage: false) : createItemList(transactions: transactionsSorted, forAnalysisPage: true, isIncomeType: false, forSelectIconPage: false);
        ...
        GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => Report(type: widget.type, category: itemList[int].text, selectedDate: widget.selectedDate, icon: iconData(itemList[int])),
              ),
            ).then((value) => setState(() {}));
          },
          child: CategoryDetails(widget.type, getTranslated(context, itemList[int].text) ?? itemList[int].text, transactionsSorted[int].amount!, transactionsSorted[int].color, iconData(itemList[int]), false),
        );
      }),
    );
  }
    ...
    return FutureBuilder<List<InputModel>>(
      ...
      builder: (BuildContext context, AsyncSnapshot<List<InputModel>> snapshot) {
        ...
        if (snapshot.data == null) {
          ...
          List<InputModel> allTransactions = filterData(context, snapshot.data!, widget.selectedDate);
          ...
          if (allTransactions.length == 0) {
            return ShowNullDetail(balance, chartDataNull, this.widget.type, true);
          } else {
            List<InputModel> transactionsSorted = [InputModel(type: this.widget.type, amount: allTransactions[0].amount, category: allTransactions[0].category)];
            ...
            while (i < allTransactions.length) {
              ...
              if (i == 1) {
                chartDataMap = {allTransactions[0].category!: allTransactions[0].amount!};
              }

              if (allTransactions[i].category == allTransactions[i - 1].category) {
                chartDataMap.update(allTransactions[i].category!, (value) => (value + allTransactions[i].amount!), ifAbsent: () => (allTransactions[i - 1].amount! + allTransactions[i].amount!));
                i++;
              } else {
                chartDataMap.addAll({allTransactions[i].category!: allTransactions[i].amount!});

                i++;
              }
              transactionsSorted = chartDataMap.entries.map((entry) => InputModel(type: this.widget.type, category: entry.key, amount: entry.value)).toList();
            }
            ...
            return Column(
              children: [
                ShowMoneyFrame(this.widget.type, this.widget.type == 'Income' ? income : expense, balance),
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        SizedBox(height: 360.h, child: connection == false ? null : ChartPie(this.chartData!)),
        CategoryDetails(this.type, getTranslated(context, 'Category') ?? 'Category', 0, this.type == 'Income' ? green : red, Icons.category_outlined, true),
      ],
    );
  }
}
...
class CategoryDetails extends StatelessWidget {
  ...
  CategoryDetails(this.type, this.category, this.amount, this.color, this.icon, this.forNullDetail);
  @override
  Widget build(BuildContext context) {
    return Card(
      color: white,
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            SizedBox(width: 10.w),
            forNullDetail ? SizedBox() : Icon(Icons.arrow_forward_ios, size: 18.sp),
          ],
        ),
      ),
    );
