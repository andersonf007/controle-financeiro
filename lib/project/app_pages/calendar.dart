...
class Calendar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: blue1, appBar: BasicAppBar(getTranslated(context, 'Calendar')!), body: CalendarBody());
  }
}
...
class _CalendarBodyState extends State<CalendarBody> {
  ...
  DateTime _focusedDay = DateTime.now(), today = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day);
  ...
  Widget buildEvents(List<InputModel>? transactions) {
    ...
    List<CategoryItem> itemList = createItemList(transactions: transactions, forAnalysisPage: false, forSelectIconPage: false, isIncomeType: false);
    return ListView.builder(
      shrinkWrap: true,
      itemCount: itemList.length,
      itemBuilder: (context, int) {
        colorCategory = transactions[int].type == 'Income' ? Colors.lightGreen : red;
        return GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => Edit(inputModel: transactions[int], categoryIcon: iconData(itemList[int])),
              ),
            ).then((value) => setState(() {}));
          },
            ...
            trailingActions: <SwipeAction>[
              SwipeAction(
                ...
                onTap: (CompletionHandler handler) {
                  Platform.isIOS
                      ? iosDialog(context, 'Are you sure you want to delete this transaction?', 'Delete', () {
                          DB.delete(transactions[int].id!);
                          setState(() {});
                          customToast(context, 'Transaction has been deleted');
                        })
                      : androidDialog(context, 'Are you sure you want to delete this transaction?', 'Delete', () {
                          DB.delete(transactions[int].id!);
                          setState(() {});
                          customToast(context, 'Transaction has been deleted');
                        });
                ...
                Container(
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(iconData(itemList[int]), color: colorCategory),
                        SizedBox(
                          width: 150.w,
                          child: Padding(
                            padding: EdgeInsets.only(left: 15.w, right: 10.w),
                            child: Text(
                              getTranslated(context, itemList[int].text) ?? itemList[int].text,
                        ...
                        Flexible(
                          ...
                          child: Padding(
                            ...
                            child: Text(
                              format(transactions[int].amount!) + ' ' + currency,
                              style: GoogleFonts.aBeeZee(fontSize: format(transactions[int].amount!).length > 15 ? 16.sp : 17.sp),
                              overflow: TextOverflow.ellipsis,
                            ),
                          ),
                        ),
                        Icon(Icons.arrow_forward_ios, size: 15.5.sp),
                      ],
                    ),
                  ),
                ),
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.data != null) {
          for (int i = 0; i < snapshot.data!.length; i++) {
            ...
            InputModel map1 = InputModel(id: snapshot.data![i].id, type: snapshot.data![i].type, amount: snapshot.data![i].amount, category: snapshot.data![i].category, description: description, date: snapshot.data![i].date, time: snapshot.data![i].time);

            void updateMapValue<K, V>(Map<K, List<V>> map, K key, V value) => map.update(key, (list) => list..add(value), ifAbsent: () => [value]);

            updateMapValue(map, '${snapshot.data![i].date}', map1);
          }
          transactions = map.map((key, value) => MapEntry(DateFormat('dd/MM/yyyy').parse(key), value));
        }

        late LinkedHashMap linkedHashedMapTransactions = LinkedHashMap<DateTime, List<InputModel>>(equals: isSameDay, hashCode: getHashCode)..addAll(transactions);

        List<InputModel> transactionsForDay(DateTime? day) => linkedHashedMapTransactions[day] ?? [];
        ...
        List<InputModel> _getEventsForRange(DateTime start, DateTime end) {
          ...
          return [for (final d in days) ...transactionsForDay(d)];
        }
        ...
        return Column(
          children: [
            TableCalendar<InputModel>(
              availableCalendarFormats: {CalendarFormat.month: getTranslated(context, 'Month')!, CalendarFormat.twoWeeks: getTranslated(context, '2 weeks')!, CalendarFormat.week: getTranslated(context, 'Week')!},
              ...
              headerStyle: HeaderStyle(
                ...
                formatButtonDecoration: BoxDecoration(boxShadow: [BoxShadow()], color: blue2, borderRadius: BorderRadius.circular(25.r)),
              ),
              calendarBuilders: CalendarBuilders(
                selectedBuilder: (context, date, _) {
                  return Container(
                    ...
                    width: 46.w,
                    height: 46.h,
                    child: Text('${date.day}', style: TextStyle().copyWith(fontSize: 17.0.sp)),
                  );
                },
                ...
                markerBuilder: (context, date, events) {
                  if (events.isNotEmpty) {
                    return Positioned(right: 1.w, bottom: 1.h, child: _buildEventsMarker(date, events));
                  }
                },
              ),
...
Widget _buildEventsMarker(DateTime date, List events) {
  ...
  return AnimatedContainer(
    ...
    decoration: BoxDecoration(shape: BoxShape.rectangle, color: Color.fromRGBO(67, 125, 229, 1)),
    width: width,
    height: 18.0.h,
    child: Center(
      child: Text(
        '${events.length}',
        style: TextStyle().copyWith(color: white, fontSize: 13.0.sp),
      ),
    ),
  );
}
    ...
    Widget summaryFrame(String type, double amount, color) => Column(
      ...
      children: [
        ...
        Text(
          ...
          style: TextStyle(color: color, fontSize: 15.sp, fontStyle: FontStyle.italic, fontWeight: FontWeight.bold),
        ),
        Text(
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            summaryFrame('INCOME', income, Colors.lightGreen),
            Padding(
              padding: EdgeInsets.symmetric(horizontal: 5.w),
              child: summaryFrame('EXPENSE', expense, red),
            ),
            Flexible(child: summaryFrame('TOTAL BALANCE', balance, Colors.black)),
          ],
        ),
      ),
    );
